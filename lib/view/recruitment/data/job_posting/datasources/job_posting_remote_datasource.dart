...
class JobPostingRemoteDatasource {
  ...
  Future<List<JobPostingModel>> getAll() => runSupabaseRemote(() async {
        final res = await _client.from(_table).select().order('created_at');
        final rows = (res as List).cast<Map<String, dynamic>>();
        return rows.map(JobPostingModel.fromDbJson).toList();
      });

  Future<JobPostingModel?> getById(String id) => runSupabaseRemote(() async {
        final row =
  ...
  Future<void> add(JobPostingModel job) => runSupabaseRemote(() async {
        await _client.from(_table).insert(job.toDbInsertJson());
      });

  Future<void> update(JobPostingModel job) => runSupabaseRemote(() async {
        await _client
            .from(_table)
  ...
  Future<void> delete(String id) => runSupabaseRemote(() async {
  ...
  Future<void> setJobActive(String id, bool isActive) =>
      runSupabaseRemote(() async {
        await _client
            .from(_table)
            .update({'is_active': isActive}).eq('job_id', id);
      });

  Future<List<String>> getJobDepartments() => runSupabaseRemote(() async {
        final res =
  ...
  Future<Map<String, int>> getApplicationCountsByJobBusinessId() =>
      runSupabaseRemote(() async {
