...
''';
  ...
  Future<JobApplicationModel?> shortlist(String applicationId) async {
    ...
    if (!ApplicationStatusActions.canUpdateStatus(current['status']?.toString())) {
  ...
  Future<JobApplicationModel?> reject(String applicationId) async {
    final current = await _client
        .from(_applicationsTable)
        .select('id, status')
        .eq('id', applicationId)
